...
fn main() {
    ...
    for value in wire2 {
        let (_trek_length, _x, _y, _gridmap) =
            parse_entry(Wire::W2, gridmap, value.to_string(), trek_length, x, y);
        trek_length = _trek_length;
        x = _x;
        y = _y;
        gridmap = _gridmap;
    ...
    let mut distance = 99999999;
    let mut combined_distance = 99999999;

    for (key, value) in &gridmap {
        if value.wire1 == true && value.wire2 == true {
            let strings: Vec<&str> = key.split("-").collect();
            let x: i32 = strings[0].parse().unwrap();
            let y: i32 = strings[1].parse().unwrap();
            let new_distance = (ORIGIN as i32 - x).abs() + (ORIGIN as i32 - y).abs();
            if new_distance < distance && new_distance > 0 {
                distance = new_distance;
            }
            
            let new_combined_distance: i32 = value.wire1_distance + value.wire2_distance;
            if new_combined_distance < combined_distance && new_combined_distance > 0 {
                combined_distance = new_combined_distance;
            }
        }
    }

    println!("Part 1 distance: {}", distance);
    println!("Part 2 distance: {}", combined_distance);
}

fn key(str1: i32, str2: i32) -> String {
    return format!("{}-{}", str1, str2);
}

fn parse_entry(
    wire: Wire,
    mut gridmap: HashMap<String, Point>,
    value: String,
    mut trek_length: i32,
    mut x: i32,
    mut y: i32,
) -> (i32, i32, i32, HashMap<String, Point>) {
    let direction = &value[0..1];
    let distance: i32 = value[1..].parse().unwrap();
    match direction {
        "U" => {
            for n in ((y - distance)..=y - 1).rev() {
                y = n;
                trek_length = trek_length + 1;
                let xy = key(x, y);
                let option = gridmap.remove(&xy);
                gridmap.insert(xy, get_wire1_point(option, trek_length, &wire));
            }
        }
        "D" => {
            for n in 1 + y..=(y + distance) {
                y = n;
                trek_length = trek_length + 1;
                let xy = key(x, y);
                let option = gridmap.remove(&xy);
    ...
    return (trek_length, x, y, gridmap);
}

fn get_wire1_point(option: Option<Point>, trek_length: i32, wire: &Wire) -> Point {
    let new_wire: Point;
    match wire {
        Wire::W1 => match option {
            Some(result) => {
                new_wire = Point {
                    wire1: true,
                    wire1_distance: trek_length,
                    ..result
                }
            }
            None => {
        ...
        Wire::W2 => match option {
            Some(result) => {
                new_wire = Point {
                    wire2: true,
                    wire2_distance: trek_length,
                    ..result
                }
            }
            None => {
