        ...
        match direction {
            "U" => {
                for n in ((y - distance)..=y- 1).rev() {
                    ...
                    gridmap.insert(String::from(xy), get_wire1_point(option));
                }
            }
            "D" => {
        ...
        match direction {
            "U" => {
                for n in ((y - distance)..=y - 1).rev() {
                    ...
                    let xy = format!("{}-{}", x, y);
                    let option = gridmap.remove(&xy);
                    gridmap.insert(String::from(xy), get_wire2_point(option));
                }
            }
            "D" => {
                for n in 1 + y..=(y + distance) {
...
fn get_wire1_point(option: Option<Point>) -> Point {
    let new_wire: Point;
    match option {
        Some(result) => {
            new_wire = Point {
...
fn get_wire2_point(option: Option<Point>) -> Point {
    let new_wire: Point;
    match option {
        Some(result) => {
            new_wire = Point {
